WITH observation_verification_status AS (SELECT        id, name
                                                                                           FROM           [StgPanda].[ventiemr].[observation_verification_status] AS observation_verification_status_1)
    SELECT        lo.encounter_id, lo.encounter_procedure_id, lo.order_number, lo.department_id, ovs.name AS lab_order_verification_status, lo.note AS lab_order_note, lo.created_at AS lab_order_created_at, loi.id AS lab_order_item_id, 
     ...
     FROM            [StgPanda].[ventiemr].[lab_order] AS lo LEFT OUTER JOIN
                              [StgPanda].[ventiemr].[observation_verification_status] AS ovs ON ovs.id = lo.verification_status_id LEFT OUTER JOIN
                              [StgPanda].[ventiemr].[lab_order_item] AS loi ON loi.lab_order_number = lo.order_number LEFT OUTER JOIN
                              [StgPanda].[ventiemr].[observation_verification_status] AS ovs2 ON ovs2.id = loi.verification_status_id LEFT OUTER JOIN
                              [StgPanda].[ventiemr].[lab_order_specimen] AS los ON los.lab_order_item_id = loi.id LEFT OUTER JOIN
                              [StgPanda].[ventiemr].[observation_verification_status] AS ovs3 ON ovs3.id = los.verification_status_id
